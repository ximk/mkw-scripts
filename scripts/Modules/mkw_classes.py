...
class KartState:
    ...
    def bitfield_0(playerIdx=0):
        return chase_pointer(getKartObjectHolder(), [0x20, playerIdx * 0x4, 0x10, 0x1C, 0x4], 'u32')

    def bitfield_1(playerIdx=0):
        return chase_pointer(getKartObjectHolder(), [0x20, playerIdx * 0x4, 0x10, 0x1C, 0x8], 'u32')

    def bitfield_2(playerIdx=0):
        return chase_pointer(getKartObjectHolder(), [0x20, playerIdx * 0x4, 0x10, 0x1C, 0xC], 'u32')

    def bitfield_3(playerIdx=0):
        return chase_pointer(getKartObjectHolder(), [0x20, playerIdx * 0x4, 0x10, 0x1C, 0x10], 'u32')

    def bitfield_4(playerIdx=0):
        return chase_pointer(getKartObjectHolder(), [0x20, playerIdx * 0x4, 0x10, 0x1C, 0x14], 'u32')

    def airtime(playerIdx=0):
        return chase_pointer(getKartObjectHolder(), [0x20, playerIdx * 0x4, 0x10, 0x1C, 0x1C], 'u32')
...
def getRaceDataHolder(playerIdx=0):
    id = get_game_id()
    address = {"RMCE01": 0x809B8F68 + (playerIdx * 0x4), "RMCP01": 0x809BD728 + (
        playerIdx * 0x4), "RMCJ01": 0x809BC788 + (playerIdx * 0x4), "RMCK01": 0x809ABD68 + (playerIdx * 0x4)}
    return address[id]
...
def getRaceInfoHolder(playerIdx=0):
    id = get_game_id()
    address = {"RMCE01": 0x809B8F70 + (playerIdx * 0x4), "RMCP01": 0x809BD730 + (
        playerIdx * 0x4), "RMCJ01": 0x809BC790 + (playerIdx * 0x4), "RMCK01": 0x809ABD70 + (playerIdx * 0x4)}
    return address[id]
...
class InputMgr:
    ghost_controllers: list

    def __init__(self):
        for i in range(4):
            self.ghost_controllers[i] = GhostController(i)
...
class GhostController:
    class InputStream:
        """
        Class representing an input stream in the ghost controller. Expected size of the buffer is 0x276c.

        Attributes:
            buffer (int): Address of the buffer inputs are read from.
            sequence_count (int): Number of unique sequences in the stream.
            size (int): Size of the buffer inputs are read from.
            state (int): State of the stream. For writing, 0 represents being safe to write; 2 represents being unsafe to write.
            name (str): Identifier for the input stream. This is used for various hacks and should be removed later on.
        """
        buffer: int
        sequence_count: int
        size: int
        ...
        def __init__(self, addr: int, name: str, write: bool):
            """
            Creates a new InputStream object.

            Args:
                addr (int): Address of the game class in memory.
                name (str): Identifier for the input stream.
                write (bool): Toggles setting the sequence count and state for writing in Python.

            Returns:
                The initialized object.
            """
            self.buffer = memory.read_u32(addr + 0x4)
            self.sequence_count = memory.read_s32(addr + 0x8)
            self.size = memory.read_u32(addr + 0xc)
            self.state = memory.read_s32(addr + 0x14)
            self.name = name

            print(f"{name} debugging information:\naddr={addr}\nbuffer={self.buffer}\nsequence_count={self.sequence_count}\nsize={self.size}\nstate={self.state}")
        ...
        def get_sequence_size(self) -> int:
            """
            Returns the size of the sequence. In all cases in the base game, this value is 2.
            """
            return 2

        @staticmethod
        def mask_buttons(accel: bool, brake: bool, item: bool, prev_mask: int) -> int:
            """
            Generates a mask of the buttons based on Frame data and the previous mask.

            Args:
                accel (bool): Acceleration input.
                brake (bool): Brake input.
                item (bool): Item input.
                prev_mask (int): Used to handle the drift mask.

            Returns:
                Mask of the provided buttons.
            """
            # NOTE (vabold): What the hell?
            drift_mask = 0x0
            if accel and brake and prev_mask not in [0x2, 0x3, 0x7]:
                drift_mask = 0x8

            return accel * 0x1 + brake * 0x2 + item * 0x4 + drift_mask

        @staticmethod
        def mask_stick(x: int, y: int) -> int:
            """
            Generates a mask of the stick based on Frame data.

            Args:
                x (int): Horizontal stick input.
                y (int): Vertical stick input.

            Returns:
                Mask of the provided stick.
            """
            return (x << 4) + y

        @staticmethod
        def mask_dpad(up: bool = False, down: bool = False, left: bool = False, right: bool = False) -> int:
            """
            Generates a mask of the dpad based on Frame data. Asserts that only one direction is True.
            ...
            Returns:
                Mask of the provided dpad inputs.
            """
            assert sum([up, down, left, right]) <= 1
        ...
        def write_frame(self, frame_value: int) -> None:
            """
            Writes the provided value to the stream.

            Args:
                frame_value (int): Value to write.
            ...
            current_sequence_index = self.sequence_count
            sequence_size = self.get_sequence_size()
            current_sequence_buffer = self.buffer + current_sequence_index * sequence_size
            new_sequence = False

            if current_sequence_index >= 0:
                if self.write_is_new_sequence(current_sequence_buffer, frame_value):
                    new_sequence = True
                    self.sequence_count += 1
                    if self.size < self.sequence_count * sequence_size:
                        # It's no longer safe to write to the buffer!
                        self.state = 2
                        return
            else:
            ...
            if current_sequence_index < 0:
                # NOTE (vabold): If this ever gets reached, could you let me know?
                return
            ...
            if new_sequence:
                self.write_sequence_start(current_sequence_buffer, frame_value)

            self.write_increment_sequence_frames(current_sequence_buffer)

        def write_increment_sequence_frames(self, buffer: int) -> None:
            """
            Increment the number of frames in the current sequence.

            Args:
                buffer (int): Address of the current sequence.

            Returns:
                None.
            """
            # HACK: We are currently relying on a name member for functions instead of inheritance.
            if self.name == 'dpad':
                current_sequence = memory.read_u16(buffer)
                frames = (current_sequence & 0xFFF) + 1
                memory.write_u16(buffer, (frames & 0xFFF) |
                                 (current_sequence & 0xF000))
            else:
                frames = memory.read_u8(buffer + 1)
                memory.write_u8(buffer + 1, frames)

        def write_is_new_sequence(self, buffer: int, frame_value: int) -> bool:
            """
            Determine whether or not a new sequence should be written.

            Args:
                buffer (int): Address of the current sequence.
                frame_value (int): Value to check against.

            Returns:
                If a new sequence should be written.
            """
            # HACK: We are currently relying on a name member for functions instead of inheritance.
            if self.name == 'dpad':
                current_sequence = memory.read_u16(buffer)
                target_value = current_sequence >> 12
                size = current_sequence & 0xFFF
                return frame_value != target_value or size == 0xFFF
            else:
                target_value = memory.read_u8(buffer)
                size = memory.read_u8(buffer + 1)
                return frame_value != target_value or size == 0xFF

        def write_sequence_start(self, buffer: int, frame_value: int) -> None:
            """
            Writes a new sequence to the buffer.

            Args:
                buffer (int): Address of the new sequence.
                frame_value (int): Value to write.
            ...
            # HACK: We are currently relying on a name member for functions instead of inheritance.
            if self.name == 'dpad':
                memory.write_u16(buffer, frame_value << 12)
            else:
                memory.write_u8(buffer, frame_value)
                memory.write_u8(buffer + 1, 0)

    controller_idx: int
    face_button_stream: InputStream
    stick_input_stream: InputStream
    direction_button_stream: InputStream

    def __init__(self, controller_idx: int):
        assert controller_idx <= 3 and controller_idx >= 0

        print(f"InputMgr chain: {InputMgr.chain()}")
        print(f"GhostController chain: {self.chain(controller_idx)}")

        self.controller_idx = controller_idx
        self.face_button_stream = self.InputStream(
            memory.read_u32(self.chain(controller_idx) + 0x94), 'buttons', True)
        self.stick_input_stream = self.InputStream(
            memory.read_u32(self.chain(controller_idx) + 0x98), 'stick', True)
        self.direction_button_stream = self.InputStream(
            memory.read_u32(self.chain(controller_idx) + 0x9c), 'dpad', True)

    @staticmethod
    def chain(controller_idx=0):
        assert controller_idx <= 3 and controller_idx >= 0
        return InputMgr.chain() + 0x3e60 + (0x2a0 * controller_idx)
